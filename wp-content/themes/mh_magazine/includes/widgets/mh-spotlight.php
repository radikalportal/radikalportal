...
class mh_spotlight_hp_widget extends WP_Widget {
    function __construct() {
		parent::__construct(
			'mh_spotlight_hp', esc_html__('MH Spotlight Widget (Homepage)', 'mh'),
    ...
    function widget($args, $instance) {
		...
		while ($spotlight_loop->have_posts()) : $spotlight_loop->the_post(); ?>
			...
			<?php if ($meta == 0) { ?>
				<p class="meta"><?php $category = get_the_category(); printf(_x('by %1$s in %2$s', 'post meta', 'mh'), get_the_author(), $category[0]->cat_name); ?></p>
			<?php } ?>
			<?php if ($excerpt == 0) { ?>
